...
microservices for MVP). Development happens on Windows 11 with the repository on a Windows drive (the drive
letter is environment-specific, e.g. `G:`) and Docker Desktop using the WSL2 backend. Persistent database storage on Windows
...
2. **Windows + WSL2 dev rules (normative):**
   - Persistent data uses **named volumes** created inside the WSL2 VM
     ...
     into a Windows drive filesystem (e.g. `G:`) for DB datadirs** (mmap/flock/sync on 9P are
     unreliable/slow).
